...
const ExploreCard = ({id,imgUrl,Url, title, index, active, handleClick}) => {
  ...
  >
    ...
    {active !== id ? (
      ...
      <div className="absolute bottom-0 p-8 justiy-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        ...
        {/* <button >Enter in the Virtul Lab</button> */}
        <p className="font-normal text-[16px] leading-[20px] text-white uppercase" onClick={() => router.push(Url)} >
          Enter in the Virtul Lab
        </p>
        <h2 className="mt-[24px] fonts-semibold sm:text-[32px] text-[24px] text-white">{title}</h2>
      </div>
    ) }
  </motion.div>
);
